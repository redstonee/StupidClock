...
namespace Graphics
{
  ...
  void networkButtonHandler(lv_event_t *e)
  {
    ...
    {
      auto circle = lv_spinner_create(btn);
  ...
  void statusRefresher(void *pvParameters)
  {
    auto objects = (lv_obj_t **)pvParameters;
    auto timeLabel = objects[0];
    auto networkBtn = objects[1];

    auto removeSpinner = [&networkBtn]()
    {
      auto c = lv_obj_get_child_count_by_type(networkBtn, &lv_spinner_class);
      if (c)
      {
        for (uint8_t i = 0; i < c; i++)
          lv_obj_delete(lv_obj_get_child_by_type(networkBtn, i, &lv_spinner_class));
      }
    };

    while (true)
    {
      auto networkStatus = Network::getWiFiStatus();
      Serial.println(static_cast<int>(networkStatus));
      switch (networkStatus)
      {
      case Network::WiFiStatus::CONNECTED:
        lv_imagebutton_set_state(networkBtn, LV_IMAGEBUTTON_STATE_RELEASED);
        lv_imagebutton_set_src(networkBtn, LV_IMAGEBUTTON_STATE_RELEASED, nullptr, &wifi_connected, nullptr);
        removeSpinner();
        break;

      case Network::WiFiStatus::DISCONNECTED:
        lv_imagebutton_set_state(networkBtn, LV_IMAGEBUTTON_STATE_RELEASED);
        lv_imagebutton_set_src(networkBtn, LV_IMAGEBUTTON_STATE_RELEASED, nullptr, &wifi_disconnected, nullptr);
        removeSpinner();
        break;

      case Network::WiFiStatus::CONNECTING:
        lv_imagebutton_set_state(networkBtn, LV_IMAGEBUTTON_STATE_DISABLED);
        break;
      }
  ...
  void drawUI()
  {
    ...
    lv_obj_add_event_cb(
        networkBtn, networkButtonHandler,
        LV_EVENT_CLICKED, nullptr);

    lv_obj_t *objToRefresh[]{timeLabel, networkBtn};
    xTaskCreate(statusRefresher, "statusRefresher", 4096, objToRefresh, 1, nullptr);
  }
} // namespace Graphics